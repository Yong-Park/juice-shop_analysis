...
async function verify(req: Request, res: Response) {
  ...
  try {
    ...
    if (!isValid) {
      await logEvent('2fa_verification', {
        userId: user.id,
        username: user.email,
        status: 'failure',
...
async function setup(req: Request, res: Response) {
  try {
    ...
    if (user.password !== security.hash(password)) {
      throw new Error('Password doesn\'t match stored password');
    }
    ...
    if (!otplib.authenticator.check(initialToken, secret)) {
      throw new Error('Initial token doesn\'t match the secret from the setupToken');
    }
